...
class PSDataset(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        gt    = np.transpose(temp['I_MS'].astype('float32'), [2,0,1]) /2047.
        pan   = temp['I_PAN'].astype('float32')[None,:,:] /2047.
...
class PSH5Dataset(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        pan = np.array(h5f['PAN'][key], dtype=np.float32) /2047.
        gt  = np.array(h5f['GT'][key], dtype=np.float32) /2047.
        ms  = np.array(h5f['MS'][key], dtype=np.float32) /2047.
