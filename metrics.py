...
def get_metrics_reduced(img1, img2):
    ...
    m3 = cc(img1, img2)
    m4 = sam(img1, img2)*180/3.14159256
    m5 = ergas(img1, img2)
    m6 = qindex(img1, img2, 32)
    return [m1.item(), m2.item(), m3.item(), m4.item(), m5.item(), m6.item()]


def get_metrics_full(img1, img2, img3):
    m1 = D_lambda(img1, img2)
    m2 = D_s(img1, img2, img3)
    m3 = qnr(img1, img2, img3)
    return [m1.item(), m2.item(), m3.item()]
...
def qindex(img1, img2, block_size=8):
    ...
    # img1: the pan-sharpened image, img2: the target image(ground truth)
    # (N, C, H, W) image; torch.float32 [0.,1.].

    if img1.shape != img2.shape:
        raise TypeError(f"Expected tensors of equal shapes, but got {img1.shape} and {img2.shape}")
    # notice: test batchsize must be 1
    img1 = img1.squeeze(0).cpu().numpy().transpose((1, 2, 0))  # (N,C,H,W)->(C,H,W)->(H,W,C)
...
def qnr(img_fake, img_lm, pan, satellite='QuickBird', scale=4, block_size=64, p=1, q=1, alpha=1, beta=1):
